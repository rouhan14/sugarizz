...
const PaymentMethod = ({ selected, onChange, isOrderingTime = true }) => {
  const optionClasses = (isActive) =>
    `border border-gray-300 rounded-md p-4 transition-all duration-200 ${
      isOrderingTime
        ? isActive
  ...
  return (
    <div>
      ...
      <div className="flex flex-col gap-3">
        {/* Cash on Delivery */}
        <div className={optionClasses(selected === "cod")}>
        ...
        {/* Online Payment */}
        <div className={optionClasses(selected === "online")}>
          <label className="flex items-center gap-3 cursor-pointer">
            <input
              type="radio"
              name="payment"
          ...
          <div
            className={`overflow-hidden transition-all duration-300 ease-in-out ${
              selected === "online" ? "max-h-40 mt-4" : "max-h-0"
            }`}
          >
